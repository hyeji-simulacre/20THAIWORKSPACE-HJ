...
def main() -> int:
    ...
    parser.add_argument("--dir", default=None, help="업로드할 폴더 경로 (상대 경로)")
    parser.add_argument("--select-uploadpage-id", default=None, help="업로드할 Notion 페이지 ID 직접 지정 (기본값보다 우선)")
    ...
    # 우선순위: CLI 인자 > 환경변수
    parent_page_id = args.select_uploadpage_id or os.getenv("NOTION_UPLOAD_DEFAULT_PAGE_ID")
    # --dir 옵션이 있으면 사용, 없으면 환경변수 사용
    target_dir_name = args.dir or os.getenv("NOTION_UPLOAD_DIR")

    if not target_dir_name:
        print("[ERROR] 업로드할 폴더를 지정해주세요. --dir 옵션 또는 NOTION_UPLOAD_DIR 환경변수 필요")
        return 1
    ...
    try:
        ...
        print("[ERROR] notion-client 패키지가 설치되지 않았습니다. python -m pip install -r .claude/skills/notion-up-lv1/requirements.txt")
        return 1
