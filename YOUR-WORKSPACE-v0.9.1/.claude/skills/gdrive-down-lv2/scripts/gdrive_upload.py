...
# -*- coding: utf-8 -*-
"""
Google Drive 파일/폴더 업로드 모듈 (Lv2에 포함)
- Workflow Mode: 지정 폴더에서 업로드 후 파일명 변경 ((done)_ or (fail)_)
- Legacy Mode: 단일 파일/폴더 수동 업로드
AI가 "어느 폴더의 파일을 업로드할까요?"라고 질문하여 폴더를 결정합니다.
"""
...
def run_workflow_mode(service, dry_run=False, target_folder=None, limit=0, files_filter=None, verbose=False):
    """
    워크플로우 실행 (업로드 후 파일명 변경: (done)_/(fail)_)
    AI가 지정한 폴더에서 파일을 업로드합니다.
    """
    print("="*50)
    ...
    # 설정 로드
    ready_dir = Path(os.getenv('GDRIVE_UPLOAD_DIR', '.')).resolve()  # AI가 --dir 옵션으로 지정
    
    # 레거시 폴더 (inprogress/complete/failed) 사용 안 함
    ...
    for f in ready_dir.iterdir():
        if not f.is_file() or f.name.startswith('.'):
            continue
        # 이미 처리된 파일 건너뛰기
        if f.name.startswith("(done)_") or f.name.startswith("(fail)_"):
            continue
        all_files.append(f)
    ...
    if dry_run:
        print("\n[DRY-RUN] 실제 업로드나 이름 변경은 수행되지 않습니다.")
        for f in files:
            print(f"  - [UPLOAD] {f.name} -> Drive Folder({root_folder_id})")
            print(f"  - [RENAME] {f.name} -> (done)_{f.name}")
        return
    ...
    for file in files:
        ...
        try:
            # 1. Upload
            result = upload_file(service, file, root_folder_id)

            if result:
                # 2. Success -> Rename to (done)_
                new_name = f"(done)_{file_name}"
                new_path = file.with_name(new_name)
                
                # 이미 (done)_ 파일이 있으면 타임스탬프 추가
                if new_path.exists():
                    timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
                    new_name = f"(done)_{file.stem}_{timestamp}{file.suffix}"
                    new_path = file.with_name(new_name)
            ...
            # 3. Fail -> Rename to (fail)_
            print(f"  [ERROR] {e}")
            new_name = f"(fail)_{file_name}"
            new_path = file.with_name(new_name)
            
            # 이미 (fail)_ 파일이 있으면 타임스탬프 추가
            if new_path.exists():
                timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
                new_name = f"(fail)_{file.stem}_{timestamp}{file.suffix}"
                new_path = file.with_name(new_name)
...
def main():
    parser = argparse.ArgumentParser(
        description='Google Drive에 파일/폴더 업로드 (Lv2에 포함)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
예시:
  # [권장] 워크플로우 실행 (.env 설정 필요)
