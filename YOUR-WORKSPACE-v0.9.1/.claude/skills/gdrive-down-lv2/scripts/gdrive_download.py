...
- Workflow Mode: 프로젝트 폴더로 다운로드 (10-working/{프로젝트}/gdrive/)
- Legacy Mode: 단일 폴더 수동 다운로드
AI가 프로젝트별 하위 폴더를 질문하여 적절한 위치에 저장합니다.
"""
...
def run_workflow_mode(service, target_folder=None, dry_run: bool = False, limit: int = 0, verbose: bool = False):
    """
    워크플로우 모드 실행
    AI가 "어느 프로젝트에 저장할까요?"라고 질문하여 저장 폴더를 결정합니다.
    """
    print("="*50)
    print("Google Drive 다운로드 (Workflow Mode)")
    print("="*50)

    # 설정 로드
    download_dir = Path(os.getenv('GDRIVE_DOWNLOAD_DIR', '10-working')).resolve() # 기본값 (AI는 프로젝트별 하위 폴더 질문)
...
def main():
    parser = argparse.ArgumentParser(
        description='Google Drive에서 파일/폴더 다운로드 (Lv2)',
