...
**사용 순서:**
1. ✅ **1순위**: 이 스킬(`create_scrapping_eduv`)의 워크플로우 실행
...
```powershell
# 1단계: 탐색(기본: 정적 HTML → RSS/Atom → 필요 시 Playwright)
python .claude/skills/_education_skills/create_scrapping_eduv/scripts/explore_template.py https://example.com

# (선택) 강제 모드
python .claude/skills/_education_skills/create_scrapping_eduv/scripts/explore_template.py https://example.com --mode static
python .claude/skills/_education_skills/create_scrapping_eduv/scripts/explore_template.py https://example.com --mode rss
python .claude/skills/_education_skills/create_scrapping_eduv/scripts/explore_template.py https://example.com --mode playwright

# 2단계: 스크립트 생성 (출력 형식 선택)
python .claude/skills/_education_skills/create_scrapping_eduv/scripts/create_scrapping.py ./structure.json -f json
python .claude/skills/_education_skills/create_scrapping_eduv/scripts/create_scrapping.py ./structure.json -f csv
python .claude/skills/_education_skills/create_scrapping_eduv/scripts/create_scrapping.py ./structure.json -f md
python .claude/skills/_education_skills/create_scrapping_eduv/scripts/create_scrapping.py ./structure.json -f all  # 전체

# 3단계: 생성된 스크립트 실행
python ./90_temp/91_temp_create_files/scrape_example_com.py          # Playwright
python ./90_temp/91_temp_create_files/scrape_example_com_static.py    # 정적 HTML
python ./90_temp/91_temp_create_files/scrape_example_com_feed.py      # RSS/Atom
```
...
```
- lv1_초급: "제가 알아서 스크래핑 도구를 만들까요?"
- lv2_고급: "원하는 대로 데이터를 수집하려면 구체적인 것이 좋습니다.
...
```powershell
python .claude/skills/_education_skills/create_scrapping_eduv/scripts/explore_template.py https://target-site.com
```

**출력:**
- `90_temp/91_temp_create_files/{domain}_structure_{timestamp}.json`
- (Playwright 사용 시) `90_temp/91_temp_create_files/{domain}_structure_{timestamp}_screenshot.png`
...
```powershell
python .claude/skills/_education_skills/create_scrapping_eduv/scripts/create_scrapping.py ./structure.json -f csv --data-output-dir "temp\\test\\crawling"
```

**출력:**
- `90_temp/91_temp_create_files/scrape_{domain}.py`
...
### **단계 6: 데이터 수집**
```powershell
python ./90_temp/91_temp_create_files/scrape_{domain}.py          # Playwright
python ./90_temp/91_temp_create_files/scrape_{domain}_static.py   # 정적 HTML
python ./90_temp/91_temp_create_files/scrape_{domain}_feed.py     # RSS/Atom
```

**출력:**
- `output_dir`(기본: `90_temp/91_temp_create_files/`) 아래에 `{domain}_data_{timestamp}.json|csv|md`
...
| 단계 | 파일 | 경로 |
|------|------|------|
| 탐색 | 구조 분석 JSON | `90_temp/91_temp_create_files/{domain}_structure_{timestamp}.json` |
| 탐색 | 스크린샷 | (Playwright 사용 시) `90_temp/91_temp_create_files/{domain}_structure_{timestamp}_screenshot.png` |
...
- **Python**: 3.12.4 (루트 `.python-version` 기준)
- **위치**: 루트 디렉토리에서 실행 권장

### **의존성 설치**
```powershell
python -m pip install -r .claude/skills/_education_skills/create_scrapping_eduv/requirements.txt
