#!/usr/bin/env python3
"""
Notion 페이지를 마크다운으로 다운로드 (legacy)

신규(lv2) 다운로드는 지정한 부모 페이지의 하위 페이지(1-depth)를 단일 폴더로 저장하는 아래 스크립트를 사용하세요:
  - notion_download_tree.py
...
def main():
    ...
    download_ready_dir = os.getenv("NOTION_DOWNLOAD_READY_DIR", "").strip()
    sync_root = os.getenv("NOTION_SYNC_ROOT", "").strip()
    ...
    # 출력 폴더 결정 (우선순위: NOTION_DOWNLOAD_READY_DIR → NOTION_SYNC_ROOT → temp/notion_downloads)
    repo_root = _find_repo_root(Path(__file__).resolve().parent)
    if download_ready_dir:
        output_dir = repo_root / download_ready_dir
    elif sync_root:
        output_dir = repo_root / sync_root
    else:
        output_dir = repo_root / "temp" / "notion_downloads"
