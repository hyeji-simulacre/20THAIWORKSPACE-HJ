...
### 4단계: 패키지 설치
```powershell
python -m pip install -r .claude/skills/notion-upload-lv1/requirements.txt
```
...
python .claude/skills/notion-upload-lv1/scripts/notion_upload_outbox.py --dir "20-created/21-ideas" --dry-run

# 2. 실제 업로드
python .claude/skills/notion-upload-lv1/scripts/notion_upload_outbox.py --dir "20-created/21-ideas"

# 3. (선택) 특정 파일만 업로드
python .claude/skills/notion-upload-lv1/scripts/notion_upload_outbox.py --dir "20-created/21-ideas" --files "my_test.md"
```
