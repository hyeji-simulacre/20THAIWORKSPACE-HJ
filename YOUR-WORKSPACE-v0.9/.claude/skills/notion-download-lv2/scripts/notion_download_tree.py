...
설정 파일: .env
기본 출력 폴더: NOTION_DOWNLOAD_DIR (기본값: 10-working)
AI가 프로젝트별 하위 폴더를 질문하여 10-working/{프로젝트}/notion/에 저장합니다.
"""
...
def main() -> int:
    ...
    parser.add_argument("--output-dir", help="저장할 폴더 (기본값: .env의 NOTION_DOWNLOAD_DIR)")
    ...
    root_id = os.getenv("NOTION_DOWNLOAD_DEFAULT_PAGE_ID")
    default_dir_name = os.getenv("NOTION_DOWNLOAD_DIR", "10-working") # 기본값 (AI는 프로젝트별 하위 폴더 질문)
    
    if args.output_dir:
        output_root = repo_root / args.output_dir
    else:
        output_root = repo_root / default_dir_name
