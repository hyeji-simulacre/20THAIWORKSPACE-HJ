...
flowchart TD
    ...
    START --> NOTION["Notion 다운로드"]
    START --> GDRIVE["GDrive 다운로드"]

    YT --> YT_PATH["30-collected/32-youtube/<br/>✅ 고정"]
    WEB --> WEB_PATH["30-collected/31-web-scraps/<br/>✅ 고정"]

    PDF --> PDF_Q{"용도가<br/>무엇인가요?"}
    PDF_Q -->|"독서 노트"| PDF_READ["20-created/22-reading-notes/"]
    PDF_Q -->|"프로젝트"| PDF_PROJ["10-working/{프로젝트}/"]
    PDF_Q -->|"기본값"| PDF_ORIG["원본 PDF 위치"]

    NOTION --> NOTION_Q{"어느 프로젝트에<br/>저장할까요?"}
    GDRIVE --> GDRIVE_Q{"어느 프로젝트에<br/>저장할까요?"}
    NOTION_Q --> NOTION_PATH["10-working/{프로젝트}/notion/"]
    GDRIVE_Q --> GDRIVE_PATH["10-working/{프로젝트}/gdrive/"]
...
flowchart TB
    ...
    subgraph SUB["하위 폴더 번호<br/>(상위의 십의 자리를 따름)"]
        S21["21-ideas"]
